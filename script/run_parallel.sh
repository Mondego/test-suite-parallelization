...
SUREFIRE_REPORT_PARSER=$PWD"/Surefire-report-parser"
...
if [[ ! -f "$SUMMARY_LOG" ]]; then
    echo " project, status, time (sec), date, forkCount, threadCount, reuseForks, parallel" > $SUMMARY_LOG
fi
...
for project in $(ls $DATASET_DIRECTORY); do
	...
	num=1
 	...
 	mvn -q compile exec:java -Dexec.args="$project_log_directory"
	cd $project_directory
 	...
 	if [[ "$r" == *"[ERROR]"* ]]; 
 	then 
 		echo $project" , ERROR , "${time[3]}" , "$DATE_WITH_TIME" , "$2" , "$3" , "$4" , "$5"" >> $SUMMARY_LOG
 	else
 		echo $project" , SUCCESS , "${time[3]}" , "$DATE_WITH_TIME" , "$2" , "$3" , "$4" , "$5"" >> $SUMMARY_LOG
 	fi
